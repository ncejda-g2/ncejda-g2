...
async def get_max_xkcd_number(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://xkcd.com/info.0.json") as response:
                if response.status != 200:
                    return {
                        "content": [
                            {"type": "text", "text": f"Error: HTTP {response.status}"}
                        ],
                        "is_error": True,
                    }
        ...
        return {
            "content": [
                {"type": "text", "text": f"Error fetching max XKCD number: {str(e)}"}
            ],
            "is_error": True,
        }
...
async def fetch_xkcd_comic(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [
                {"type": "text", "text": f"Error fetching XKCD comic: {str(e)}"}
            ],
            "is_error": True,
        }
...
async def download_image(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [
                {"type": "text", "text": f"Error downloading/saving image: {str(e)}"}
            ],
            "is_error": True,
        }
